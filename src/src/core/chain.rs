...
use crate::core::transaction::{Transaction, TxId, KeyImage, TxType, StealthOutput, PedersenCommitment, Bulletproof};
use crate::crypto::keys::{ViewKey, PublicKey};
...
impl Chain {
    ...
    /// Scan the UTXO set for outputs belonging to a given view key.
    /// Returns all owned UTXOs with decrypted amounts.
    pub fn scan_utxos(&self, view_key: &ViewKey, spend_pubkey: &PublicKey) -> Vec<OwnedUtxo> {
        use crate::crypto::stealth::scan_output;
        use crate::crypto::ringct::decrypt_amount;
        use curve25519_dalek::scalar::Scalar;

        let mut owned = vec![];
        for ((tx_id, idx), entry) in &self.utxo_set {
            // Skip placeholder outputs (all-zero keys from coinbase_placeholder)
            if entry.output.tx_pubkey == [0u8; 32] {
                continue;
            }

            if let Some(shared_secret_bytes) = scan_output(
                &entry.output.tx_pubkey,
                &entry.output.one_time_pubkey,
                *idx,
                view_key,
                spend_pubkey,
            ) {
                // Convert bytes back to Scalar for amount decryption
                let s_opt = Scalar::from_canonical_bytes(shared_secret_bytes);
                if s_opt.is_some().into() {
                    let s: Scalar = s_opt.unwrap();
                    if let Some(amount) = decrypt_amount(&entry.output.encrypted_amount, &s) {
                        owned.push(OwnedUtxo {
                            ...
                            one_time_pubkey: entry.output.one_time_pubkey,
                        });
                    }
                }
            }
    ...
    /// Get random UTXO public keys for ring decoys (excluding a specific output)
    pub fn get_decoy_keys(&self, exclude: &(TxId, u32), count: usize) -> Vec<[u8; 32]> {
        self.utxo_set
            .iter()
            .filter(|(k, _)| *k != exclude)
            .filter(|(_, e)| e.output.one_time_pubkey != [0u8; 32]) // skip placeholders
            .take(count)
            .map(|(_, e)| e.output.one_time_pubkey)
            .collect()
    }

    /// Get total number of UTXOs in the set
...
/// A UTXO that belongs to a specific wallet (detected via view key scanning)
#[derive(Debug, Clone)]
pub struct OwnedUtxo {
    ...
    pub amount_ncip: u64,
    pub block_height: u64,
    /// Shared secret s_i (needed to derive spend key for this output)
