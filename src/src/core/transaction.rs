...
impl TxType {
    pub fn as_byte(&self) -> u8 {
        match self {
            ...
            TxType::StakeDeposit => 4,
            TxType::StakeWithdraw => 5,
        }
    }
}
...
impl Transaction {
    ...
    /// Hash of everything except ring signatures — used for signing and verification.
    /// This is the message that ring signatures commit to.
    pub fn signing_hash(&self) -> [u8; 32] {
        let mut hasher = Sha3_256::new();
        // Hash outputs
        for output in &self.outputs {
            hasher.update(&output.one_time_pubkey);
            hasher.update(&output.tx_pubkey);
            hasher.update(&output.amount_commitment.0);
            hasher.update(&output.encrypted_amount);
        }
        // Hash fee commitment
        hasher.update(&self.fee_commitment.0);
        // Hash type + version
        hasher.update(&[self.version, self.tx_type.as_byte()]);
        let result = hasher.finalize();
        let mut hash = [0u8; 32];
        hash.copy_from_slice(&result);
        hash
    ...
    /// Create a real coinbase transaction with stealth output for the validator
    pub fn coinbase(
        recipient: &crate::crypto::keys::StealthAddress,
        reward_ncip: u64,
    ) -> Self {
        use crate::crypto::stealth::generate_output;
        use crate::crypto::ringct::{commit_random, encrypt_amount, prove_range};

        let output_keys = generate_output(recipient, 0)
            .expect("coinbase: invalid recipient stealth address");

        let c = commit_random(reward_ncip);
        let encrypted = encrypt_amount(reward_ncip, &output_keys.shared_secret);
        let range_proof = prove_range(reward_ncip, &c.blinding())
            .expect("coinbase: range proof generation failed");
        let amount_commitment = c.commitment();

        Transaction {
            tx_type: TxType::Coinbase,
            inputs: vec![],
            outputs: vec![StealthOutput {
                one_time_pubkey: output_keys.one_time_pubkey,
                tx_pubkey: output_keys.tx_pubkey,
                amount_commitment,
