...
use super::tor::OnionAddress;
use super::peer::{PeerId, PeerInfo, PeerReputation};
...
impl GossipMessage {
    pub fn new(topic: &str, payload: Vec<u8>) -> Self {
        ...
        let message_id = hasher.finalize().into();
...
impl P2PNode {
    ...
    /// Start the P2P node
    /// In production: spawns libp2p swarm over Tor transport
    pub async fn start(&mut self, our_onion: &OnionAddress) -> Result<(), P2PError> {
        info!("🌐 P2P node starting...");
        info!("📡 Listen: {}", our_onion.to_multiaddr(self.config.listen_port));
...
mod tests {
    ...
    async fn test_broadcast_after_start() {
        let (mut node, _rx) = make_node();
        let onion = super::super::tor::OnionAddress::new(
            "cipherxnode1234567890abcdefghijklmnop56789012345678".to_string()
        ).unwrap();
        node.start(&onion).await.unwrap();
