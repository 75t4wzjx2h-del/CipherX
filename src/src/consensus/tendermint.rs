...
impl Proposal {
    pub fn verify(&self) -> bool {
        self.validator_proof.verify()
        // TODO Phase 3: sig verify
    }
}
...
mod tests {
    ...
    fn test_quorum_single_validator() {
        ...
        let result = vs.add(Vote {
            vote_type: VoteType::Prevote, height: 1, round: 0,
            block_hash: Some(BlockHash([42u8; 32])),
            validator_nullifier: null(1), signature: vec![], voting_power: 1,
        });
    ...
    fn test_quorum_three_validators() {
        ...
        let bh = BlockHash([42u8; 32]);
        for i in 1u8..=2 {
            let r = vs.add(Vote {
                vote_type: VoteType::Prevote, height: 1, round: 0,
                block_hash: Some(bh.clone()),
                validator_nullifier: null(i), signature: vec![], voting_power: 1,
            });
        ...
        let r = vs.add(Vote {
            vote_type: VoteType::Prevote, height: 1, round: 0,
            block_hash: Some(bh.clone()),
            validator_nullifier: null(3), signature: vec![], voting_power: 1,
        });
        assert!(r.is_some());
    }
    ...
    fn test_duplicate_vote_ignored() {
        ...
        vs.add(Vote { vote_type: VoteType::Prevote, height: 1, round: 0,
            block_hash: bh.clone(), validator_nullifier: n, signature: vec![], voting_power: 1 });
        assert_eq!(vs.count(), 1);
        vs.add(Vote { vote_type: VoteType::Prevote, height: 1, round: 0,
            block_hash: bh.clone(), validator_nullifier: n, signature: vec![], voting_power: 1 });
        assert_eq!(vs.count(), 1); // still 1
    }
